...
export function FailureList({ items, truncated = false }) {
    if (!items.length) {
    ...
    return (_jsxs(View, { style: styles.list, children: [truncated ? (_jsxs(View, { style: styles.truncated, children: [_jsx(Text, { style: styles.truncatedTitle, children: "More failures exist" }), _jsx(Text, { style: styles.truncatedText, children: "Not all failures are loaded on this device. Failure memory remains intact." })] })) : null, items.map((item) => (_jsx(FailureCard, { item: item }, item.id)))] }));
}
