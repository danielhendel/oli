/**
 * Your Pattern — 7 / 30 / 90-day overnight lowest HR averages with personal-range status.
...
import type { RestingHeartRatePatternComparison } from "@/lib/data/readiness/buildRestingHeartRateDetailViewModel";
...
import { restingHeartRatePatternStatusTextColor } from "@/lib/ui/theme/restingHeartRatePersonalRangeChrome";
import { UI_TEXT_MUTED, UI_TEXT_PRIMARY, UI_TEXT_SECONDARY } from "@/lib/ui/theme/uiTokens";

export type RestingHeartRatePatternComparisonProps = {
  pattern: RestingHeartRatePatternComparison;
  loading?: boolean;
  testID?: string;
};
      ...
      <View style={styles.rowRight}>
        ...
        {statusLabel && !loading ? (
          <Text
            style={[
              styles.statusLabel,
              { color: restingHeartRatePatternStatusTextColor(statusLabel) },
            ]}
...
export function RestingHeartRatePatternComparisonView({
  pattern,
  loading = false,
  testID = "resting-heart-rate-pattern",
}: RestingHeartRatePatternComparisonProps): React.ReactElement {
  return (
...
export function RestingHeartRatePatternComparisonSkeleton({
  testID = "resting-heart-rate-pattern",
}: {
  testID?: string;
}): React.ReactElement {
  const placeholder: RestingHeartRatePatternComparison = {
  ...
  return <RestingHeartRatePatternComparisonView pattern={placeholder} loading testID={testID} />;
}
