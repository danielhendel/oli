/**
 * Your Pattern — 7 / 30 / 90-day readiness contributor-score averages with provider labels.
 * Presentation only; averages and classification live on the view model.
 */

import React from "react";
import { StyleSheet, Text, View } from "react-native";

import type { ReadinessContributorPatternComparison } from "@/lib/data/readiness/readinessContributorDetailTypes";
...
import { readinessContributorPatternStatusTextColor } from "@/lib/ui/theme/readinessContributorScoreChrome";
import { UI_TEXT_MUTED, UI_TEXT_PRIMARY, UI_TEXT_SECONDARY } from "@/lib/ui/theme/uiTokens";

export type ReadinessContributorPatternComparisonProps = {
  pattern: ReadinessContributorPatternComparison;
  loading?: boolean;
  testID?: string;
};
      ...
      <View style={styles.rowRight}>
        ...
        {statusLabel && !loading ? (
          <Text
            style={[
              styles.statusLabel,
              { color: readinessContributorPatternStatusTextColor(statusLabel) },
            ]}
...
export function ReadinessContributorPatternComparisonView({
  pattern,
  loading = false,
  testID = "readiness-contributor-pattern",
}: ReadinessContributorPatternComparisonProps): React.ReactElement {
  return (
...
export function ReadinessContributorPatternComparisonSkeleton({
  testID = "readiness-contributor-pattern",
}: {
  testID?: string;
}): React.ReactElement {
  const placeholder: ReadinessContributorPatternComparison = {
  ...
  return (
    <ReadinessContributorPatternComparisonView pattern={placeholder} loading testID={testID} />
  );
}
