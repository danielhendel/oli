...
import type { StrengthYearlyChartMonth } from "@/lib/data/workouts/strengthYearlyCardModel";
...
/**
 * Under Jest we deliberately avoid touching `Animated` so this card can mount inside
 * existing workouts-overview tests that stub `react-native` down to a minimal set of
 * primitives (no `Animated`/`Easing`). Production (and tests that load `jest-expo` defaults)
 * use the real animated track.
 */
const IS_JEST = typeof process.env.JEST_WORKER_ID !== "undefined";

/**
 * 12-month bar track for the Yearly Strength card.
 *
 * Sibling of {@link ActivityYearlyMonthBars} — same Oli blue (`ENERGY_BASELINE_FILL_COLOR`),
 * same rim hairline, same Jest-aware animation, same "non-current month" dimming. Kept as a
 * sibling per the audit approval (no shared chart refactor in this PR).
 *
 * Differences vs. activity:
 * - Bound to {@link StrengthYearlyChartMonth} (workout counts, not steps averages).
 * - Value labels above bars display the raw integer count (e.g. `"12"`); months with zero
 *   counts render no label.
 * - Future months render a low-opacity placeholder bar (zero-height + dim opacity).
 */
...
type Props = {
  ...
  /** Vertical scale max (workout count). */
  maxScale: number;
  /** `"YYYY-MM"` for today — used to emphasize the current month. */
  todayMonthKey: string;
  /** Optional formatter for the value label above bars (defaults to integer string). */
...
export function StrengthYearlyMonthBars({
  months,
  barTrackHeight,
  maxScale,
  todayMonthKey,
  ...
  return (
    <View style={styles.outer} testID="workouts-yearly-month-chart">
      <View style={styles.valueLabelsRow}>
        {months.map((m) => {
          const showLabel = !m.isFutureMonth && m.workoutCount > 0;
          return (
            <View key={`lab-${m.monthKey}`} style={styles.valueLabelCell}>
              {showLabel ? (
                <Text style={styles.valueLabelAbove}>{labelFor(m.workoutCount)}</Text>
              ) : null}
            </View>
          );
        })}
        ...
        <View style={styles.barsRowInner}>
          {months.map((m, idx) => {
            ...
            const a11y = m.isFutureMonth
              ...
              : value > 0
                ? `Month ${m.monthKey}, ${value} workout${value === 1 ? "" : "s"} completed`
                : `Month ${m.monthKey}, no recorded workouts`;
            const stackOpacity =
              (m.isCurrentMonth ? 1 : BAR_DIM_OPACITY_NON_CURRENT) *
              barTierOpacity(m.monthKey, todayMonthKey, value, m.isFutureMonth);
