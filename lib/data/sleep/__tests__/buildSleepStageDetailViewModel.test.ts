...
describe("buildSleepStageDetailViewModel — Deep", () => {
  it("ready with minutes hero, percent secondary, and adult context for ages 18–64", () => {
    ...
    expect(vm.adultContext?.currentMarkerPosition01).toBeDefined();
    ...
    expect(vm.accessibilitySummary).toContain("This result is below the typical range");
    expect(vm.accessibilitySummary).not.toMatch(/Today is|90-day average is \d+ percent/);
...
describe("buildSleepStageDetailViewModel — REM", () => {
  it("ready with rem minutes, percent, and within typical adult context", () => {
    ...
    expect(vm.accessibilitySummary).toContain("This result is in the typical range");
