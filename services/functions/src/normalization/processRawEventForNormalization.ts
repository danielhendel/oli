// services/functions/src/normalization/processRawEventForNormalization.ts
...
export type RawEventNormalizationTrigger = "create" | "update";

/**
 * Shared path for rawEvent → canonical + side effects (workout summaries, fact-only recompute).
 * Used by {@link onRawEventCreated} and {@link onRawEventUpdatedForNormalization}.
 */
export async function processRawEventForNormalization(params: {
  snapshot: DocumentSnapshot;
  pathUserId: string;
  rawEventId: string;
  trigger: RawEventNormalizationTrigger;
}): Promise<void> {
  const { snapshot, pathUserId, rawEventId, trigger } = params;
  ...
  logger.info("RAW_EVENT_NORMALIZATION", {
    msg: "RAW_EVENT_NORMALIZATION",
    trigger,
    ...
    if (isFactOnly) {
      ...
      try {
        logger.info("FACT_ONLY_RECOMPUTE_START", {
          ...
          userId: rawEvent.userId,
          dayKey: ctx.dayKey,
          trigger: trigger === "update" ? "onRawEventUpdated_factOnly" : "onRawEventCreated_factOnly",
        });
