// app/(app)/workout/setup/past.tsx
...
export default function WorkoutPast() {
  ...
  React.useEffect(() => {
    (async () => {
      if (!user?.uid) return;
      try {
        const rows = await fetchRecentEventsByType(user.uid, "workout", 30);
        setItems(rows);
      } catch (e) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#F4F5F7" }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
        ...
        {items.length === 0 ? (
          <Card variant="elevated" radius="lg" padding="lg"><Text>No recent workouts yet.</Text></Card>
        ) : items.map((it) => (
          <Card key={it.id} variant="elevated" radius="lg" padding="lg" style={{ gap: 8 }}>
            <Text weight="medium">{it.ymd}</Text>
            <Text tone="muted" size="sm">Reuse this workout</Text>
