...
describe("Health record placeholder pages", () => {
  ...
  it("redirects legacy Supplements placeholder to Nutrition supplements", async () => {
    let test!: renderer.ReactTestRenderer;
    await act(async () => {
      test = renderer.create(<SupplementsPlaceholderScreen />);
    });
    const redirect = test.root.findByType("Redirect" as unknown as React.ElementType);
