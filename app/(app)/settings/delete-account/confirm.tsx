...
export default function DeleteAccountConfirmScreen() {
  ...
  return (
    <ModuleScreenShell title="Confirm deletion" subtitle="Type DELETE to continue">
      <View style={{ gap: 12 }}>
        <View style={{ borderWidth: 1, borderColor: "#ddd", borderRadius: 12, padding: 12, gap: 8 }}>
          <Text style={{ fontSize: 14, fontWeight: "800" }}>This cannot be undone</Text>
          <Text>
            To delete your account and all associated data, type <Text style={{ fontWeight: "800" }}>DELETE</Text> below.
          </Text>
        </View>
                  ...
                  void (async () => {
                    ...
                    try {
                      ...
                      if (!res.ok && (res.status === 401 || res.status === 403)) {
                        ...
                        if (refreshed) res = await requestAccountDelete(refreshed, rid);
                      }
        ...
        >
          <Text style={{ color: "#fff", fontWeight: "800" }}>{submitting ? "Submitting…" : "Delete account"}</Text>
        </Pressable>
      </View>
    </ModuleScreenShell>
  );
