...
export default function CardioHub() {
  ...
  const { hasLogsMap } = useHasLogsMap("cardio", user?.uid ?? null, displayBaseYmd);

  return (
    <HubScaffold
      title="Cardio"
      onPressPlus={() => {
        const href = { pathname: "/cardio/setup", params: { ymd: selectedYmd } } satisfies Href;
        router.push(href);
      }}
      headerChildren={
        <WeekHeader
          ...
          onSelect={(ymd) => {
            setSelectedYmd(ymd);
            const href = { pathname: "/cardio/day/[ymd]", params: { ymd } } satisfies Href;
            router.push(href);
          }}
    ...
    >
      <Card variant="elevated" radius="lg" padding="md">
        <Text weight="bold" size="lg">Welcome to the Cardio Hub</Text>
        <Text tone="muted" style={{ marginTop: 6 }}>
          Rings glow neon green on days with cardio.{'\n'}
