/**
 * Overview highlights "today" on the weekly strip (not last navigated day).
 */
...
jest.mock("@/lib/ui/calendar/WeeklyStrip", () => {
  ...
  return {
    WeeklyStrip: (props: React.ComponentProps<typeof actual.WeeklyStrip>) => {
      weeklyStripCalls.push({ selectedDay: props.selectedDay });
      return <actual.WeeklyStrip {...props} />;
    },
  };
});
...
describe("Workouts overview weekly strip", () => {
  beforeEach(() => {
    weeklyStripCalls.length = 0;
  });

  it("passes today as selectedDay (not a stale navigation selection)", async () => {
    await act(async () => {
      renderer.create(<TrainingOverviewScreen />);
    });

    const last = weeklyStripCalls[weeklyStripCalls.length - 1];
    expect(last?.selectedDay).toBe("2026-03-12");
  });
});
