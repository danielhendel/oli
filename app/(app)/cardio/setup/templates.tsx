// app/(app)/cardio/setup/templates.tsx
...
export default function CardioTemplates() {
  const { ymd } = useLocalSearchParams<{ ymd?: string }>();
  const router = useRouter();
  const { user } = useAuth();
  const [items, setItems] = React.useState<Array<TemplateDoc<CardioPayload>>>([]);
  ...
  React.useEffect(() => {
    (async () => {
      if (!user?.uid) return;
      try {
        const t = await fetchTemplatesByType<CardioPayload>(user.uid, "cardio");
        setItems(t);
      } catch (e) {
  ...
  function selectTemplate(t: TemplateDoc<CardioPayload>) {
    const prefill = encodePrefill<CardioPayload>(t.payload);
    router.push({ pathname: "/cardio/log/manual", params: { ymd, prefill } });
  }

  return (
    <View style={{ flex: 1, backgroundColor: "#F4F5F7" }}>
      <DetailHeader title="Cardio Templates" onBack={onBack} />
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
        {error ? <Card variant="elevated" radius="lg" padding="lg"><Text tone="danger">{error}</Text></Card> : null}
        {items.length === 0 ? (
          <Card variant="elevated" radius="lg" padding="lg"><Text>No templates yet.</Text></Card>
        ) : items.map((t) => (
