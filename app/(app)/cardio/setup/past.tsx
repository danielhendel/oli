...
export default function CardioPast() {
  ...
  React.useEffect(() => {
    (async () => {
      if (!user?.uid) return;
      try {
        const rows = await fetchRecentEventsByType(user.uid, "cardio", 30);
        setItems(rows);
      } catch (e) {
  ...
  function onBack() {
    ...
    router.replace({ pathname: "/cardio", params: { focusYmd: day } });
  }
