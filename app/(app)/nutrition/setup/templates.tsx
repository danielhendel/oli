// app/(app)/nutrition/setup/templates.tsx
...
type NutritionPayload = { totals?: { calories?: number; protein?: number } };

export default function NutritionTemplates() {
  const { ymd } = useLocalSearchParams<{ ymd?: string }>();
  const router = useRouter();
  const { user } = useAuth();
  const [items, setItems] = React.useState<Array<TemplateDoc<NutritionPayload>>>([]);
  ...
  React.useEffect(() => {
    (async () => {
      if (!user?.uid) return;
      try {
        const t = await fetchTemplatesByType<NutritionPayload>(user.uid, "nutrition");
        setItems(t);
      } catch (e) {
  ...
  function selectTemplate(t: TemplateDoc<NutritionPayload>) {
    const prefill = encodePrefill<NutritionPayload>(t.payload);
    router.push({ pathname: "/nutrition/log/manual", params: { ymd, prefill } });
  }

  return (
    <View style={{ flex: 1, backgroundColor: "#F4F5F7" }}>
      <DetailHeader title="Nutrition Templates" onBack={onBack} />
