...
export const enrichDailyFactsWithBaselinesAndAverages = (input) => {
    ...
    if (hasPrior) {
        ...
        if (avgSteps !== undefined || avgTrainingLoad !== undefined) {
            const baseActivity = enriched.activity
                ? { ...enriched.activity }
                : undefined;
            const nextActivity = baseActivity ?? {};
            if (avgSteps !== undefined)
                nextActivity.stepsAvg7d = avgSteps;
            if (avgTrainingLoad !== undefined)
                nextActivity.trainingLoadAvg7d = avgTrainingLoad;
    ...
    if (baseline !== undefined) {
        const baseRecovery = enriched.recovery
            ? { ...enriched.recovery }
            : undefined;
        const nextRecovery = baseRecovery ?? {};
    ...
    const computeCoverage = (isPresent) => {
        if (window.length === 0)
            return undefined;
    ...
    const sleepConfidence = computeCoverage((f) => hasAnyTruthyKey(f.sleep));
    ...
    const bodyConfidence = computeCoverage((f) => hasAnyTruthyKey(f.body));
    if (sleepConfidence !== undefined ||
        activityConfidence !== undefined ||
        recoveryConfidence !== undefined ||
        bodyConfidence !== undefined) {
        const baseConfidence = enriched.confidence
            ? { ...enriched.confidence }
            : undefined;
        const nextConfidence = baseConfidence ?? {};
        if (sleepConfidence !== undefined)
            nextConfidence.sleep = sleepConfidence;
        if (activityConfidence !== undefined)
            nextConfidence.activity = activityConfidence;
        if (recoveryConfidence !== undefined)
            nextConfidence.recovery = recoveryConfidence;
        if (bodyConfidence !== undefined)
            nextConfidence.body = bodyConfidence;
