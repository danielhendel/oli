 ...
 */
export type HealthSourceType = 'wearable' | 'mobile_app' | 'manual' | 'lab' | 'device' | 'import';
/**
 * Core canonical event kinds for v1.
 * Sprint 6 adds:
 * - Nutrition daily totals (macros) as a canonical event kind.
 */
export type CanonicalEventKind = 'sleep' | 'steps' | 'workout' | 'weight' | 'hrv' | 'nutrition';
/**
 ...
 */
export type CanonicalEvent = SleepCanonicalEvent | StepsCanonicalEvent | WorkoutCanonicalEvent | WeightCanonicalEvent | HrvCanonicalEvent | NutritionCanonicalEvent;
/**
...
export declare function isYmdDateString(value: string): boolean;
/**
 * Type guard to distinguish CanonicalEvent from RawEvent based on
 * the presence of canonical-only fields.
 */
export declare function isCanonicalEvent(event: CanonicalEvent | RawEvent): event is CanonicalEvent;
/** Minimal runtime check for DailyFacts shape */
export declare function isDailyFacts(value: unknown): value is DailyFacts;
