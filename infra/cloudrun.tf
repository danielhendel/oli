resource "google_cloud_run_v2_service" "api" {
  ...
  template {
    ...
    containers {
      ...
      env { name = "EXPORTS_BUCKET" value = google_storage_bucket.data_exports.name }
  ...
  ingress = "INGRESS_INTERNAL_AND_EXTERNAL"
  labels  = { env = var.env, app = "healthos" }
}

# Require authentication (no unauthenticated invoker).
# Callers (mobile app or gateway) must present Firebase ID token to the API itself.
