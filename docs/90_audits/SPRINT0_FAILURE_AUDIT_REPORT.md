...
This was **not a single bug**. It was a chained system failure spanning multiple layers of the platform:
...
- No `rawEvent` document was written to Firestore.

**Fix**
- Added `timeZone` to the ingest body in `lib/api/usersMe.ts`.

---

### 2) Eventarc → Cloud Run authentication failure (critical)

Even after ingest succeeded, Gen2 trigger delivery was blocked and compute never executed.

**Evidence**
- Cloud Run request logs for the Gen2 backend `onraweventcreated` showed repeated `403` responses:
